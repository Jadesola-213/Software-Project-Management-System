...
const CardModal = ({ cardId, open, setOpen, card, list }) => {
  ...
  const onTitleDescriptionSubmit = async (e) => {
    e.preventDefault();
    dispatch(editCard(cardId, { title, description, deadline }));
  };
    ...
    <Modal open={open} onClose={() => setOpen(false)}>
      <div className={`${classes.paper} ${classes.cardModal}`}>
        <form onSubmit={(e) => onTitleDescriptionSubmit(e)}>
          ...
          />
          <h3 className={classes.membersTitle}>Deadline</h3>
           <DatePicker
           style={{marginBottom: '20px'}}
            variant='outlined'
            margin='normal'
            label='Deadline'
            placeholderText="Select deadline for this card"
            minDate={new Date()}
            selected={deadline} onChange={(newDate) => setDeadline(newDate)} />
          <Button
            type='submit'
            variant='contained'
            color='primary'
