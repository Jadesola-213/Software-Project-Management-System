...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      list.files.push(req.body.title);
      await list.save();

      // Log activity
      const user = await User.findById(req.user.id);
      board.activity.unshift({
        text: `${user.name} uploaded a file to this board`,
      });
      await board.save();
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      list.files.splice(req.body.title);
      await list.save();

      // Log activity
      const user = await User.findById(req.user.id);
      board.activity.unshift({
        text: `${user.name} deleted a file to this board`,
      });
      await board.save();
...
// Delete a list
router.patch("/delete/:id", [auth, member], async (req, res) => {
  try {
    const list = await List.findOneAndDelete({ _id: req.params.id });

    // Delete the list from the board
    const board = await Board.findOne({ lists: req.params.id });
    board.lists.map(async (d, i) => {
      if (d == req.params.id) {
        board.lists.splice(i);
        await board.save();
      }
    });
    ...
    // Log activity
    const user = await User.findById(req.user.id);
    const board2 = await Board.findById(req.header("boardId"));
    board2.activity.unshift({
      text: `${user.name} deleted list '${list.title}'`,
    });
    await board2.save();
